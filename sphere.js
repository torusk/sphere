...
let workDuration = 25 * 60; // 25分作業
let breakDuration = 5 * 60; // 5分休憩
let countdownSeconds = workDuration; // 初期状態は作業時間
let isWorkTime = true; // 現在が作業時間か休憩時間かを追跡
...
renderer.domElement.addEventListener("click", function () {
  if (!countdownActive) {
    ...
    isWorkTime = true; // クリック時は必ず作業時間から開始
    countdownSeconds = workDuration; // 作業時間をセット
    updateTimerDisplay(formatTime(countdownSeconds)); // 表示を更新
  ...
  if (currentTime - lastCountdownUpdate >= 1000) {
    ...
    if (countdownSeconds <= 0) {
        ...
        playAlarmSound().then((success) => {
          if (success) {
            ...
            if (timerAudio) {
              ...
              timerAudio
                .play()
                .catch((e) => console.error("バックアップ再生にも失敗:", e));
            }
          }
          hasPlayedSound = true; // 次の音のためにリセット
        });
      }
      ...
      if (isWorkTime) {
        // 作業時間が終了、休憩時間を開始
        isWorkTime = false;
        countdownSeconds = breakDuration;
        console.log("作業終了、休憩開始");
      } else {
        // 休憩時間が終了、作業時間を開始
        isWorkTime = true;
        countdownSeconds = workDuration;
        console.log("休憩終了、作業開始");
      }
      hasPlayedSound = false; // 次のセッションで音が鳴るようにする
      lastCountdownUpdate = Date.now(); // 時間切り替え後の即時ティックを防ぐために更新時間をリセット
      // countdownActive はサイクルの継続のために true のまま
      // releaseWakeLock(); // セッション間でスリープ防止を解除/再取得するか検討
      // 連続的なポモドーロのためにはアクティブのままにしておくのが良いかもしれません
    }
